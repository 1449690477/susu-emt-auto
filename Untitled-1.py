...
def resolve_game_directory() -> str:
    external_game = os.path.join(APP_DIR, "Game")
    internal_game = os.path.join(DATA_DIR, "Game")

    if os.path.isdir(external_game):
        return external_game
    if os.path.isdir(internal_game):
        return internal_game

    ensure_directory(external_game)
    return external_game
